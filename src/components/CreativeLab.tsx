...
import { ExternalLink, Code, Zap, Brain, Eye, Wifi } from "lucide-react"
...
const categories = ["All", "3D", "AI", "Realtime"]
...
export default function CreativeLab() {
  ...
  const filteredExperiments =
    activeCategory === "All" ? experiments : experiments.filter((exp) => exp.category === activeCategory)
      ...
      <div className="w-full max-w-7xl mx-auto px-6 lg:px-12">
        <motion.div ref={ref} initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants}>
          ...
          <motion.div variants={itemVariants} className="text-center mb-2xl">
            <h2 className="text-section font-bold mb-lg gradient-text">Creative Lab</h2>
            <p className="text-xl text-text-secondary max-w-2xl mx-auto mb-lg">
              Experimental projects exploring the boundaries of creative technology, where innovation meets imagination
            </p>
                  ...
                  <motion.button
                    ...
                    className={`inline-flex items-center px-4 py-2 rounded-full text-small font-medium transition-all duration-300 ${
                      activeCategory === category
                        ? "bg-accent-cyan text-black shadow-glow-md"
          ...
          {filteredExperiments.length === 0 ? (
            <motion.div variants={itemVariants} className="text-center py-12">
              <p className="text-xl text-text-secondary">No experiments found in this category.</p>
            </motion.div>
          ) : (
            <>
              {/* Projects Grid */}
              <motion.div variants={itemVariants} className="mb-xl">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
                  {filteredExperiments.map((experiment) => {
                    ...
                    return (
                      <motion.div key={experiment.id} variants={itemVariants} className="group" layout>
                        <motion.div
                        ...
                        >
                          ...
                          <div className="relative z-10">
                            ...
                            <p className="text-body text-text-secondary mb-md">{experiment.description}</p>
          ...
          {/* CTA */}
          <motion.div variants={itemVariants} className="text-center mt-2xl card-glass rounded-card p-xl hover-glow">
            <h3 className="text-2xl font-semibold mb-sm gradient-text">Collaborate on something innovative?</h3>
